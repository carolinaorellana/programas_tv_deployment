...
@app.route('/programas/<int:id>')
def bienvenido_segun_id(id):
    print(id)
    if session['login']==True:
        todos_los_programas=Programa.todos_los_programas()
        return render_template("programas.html", todos_los_programas= todos_los_programas)
    else:
        return redirect ('/') 
...
def registrar_usuario():
    ...
    session['login']=True
    return redirect (f"/programas/{usuario_id}")
...
def iniciar_usuario():
    ...
    if not Usuario.validacion_login(data):
        ...
        #esta es para saber que si session login es true ya se inicio la sesion de algun registrado
        session['login']=True
        return redirect (f"/programas/{usuario_existente.id}")
